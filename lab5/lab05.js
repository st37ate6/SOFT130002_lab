...
function showWindowHref() {
    ...
    var parser = document.createElement("a"); // create an anchor in document.
    parser.href = urlContent;


    var queryString = parser.search; // get query string.
    const parameters = new URLSearchParams(queryString);
    url_result.value = parameters.get("name");
}
...
function arrSameStr() {
    ...
    string.split("").forEach(function(char) {
        if (string.split(char).length > max) {
            ...
            // example: a string "aaababb", split by a, is "" "" "" "b" "bb", length = #a + 1.
            maxChar = char;
        }
    })
