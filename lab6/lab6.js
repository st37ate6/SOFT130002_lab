...
function testTime() {
    ...
    var timer = setInterval(function() {
        if (count > 10 || new Date().getMinutes() !== startMinute) {
            console.log("The function stops.")
            clearInterval(timer);
            readInput();
        } else {
            count++;
...
要求：
    ...
    ④telephone与mail均是字符串。
*/
// testMail("18917384298", "fsy2001@outlook.com");

function testMail(telephone, mail) {
    var telValid, mailValid;

    var emailCheck = new RegExp(/^(([^<>()\[\]\\.,;:\s@"]+(\.[^<>()\[\]\\.,;:\s@"]+)*)|(".+"))@(([a-zA-Z0-9]+(-[a-zA-Z0-9]+)*)\.)+((?![0-9]+$)[a-zA-Z0-9]+)$/),
        telCheck = new RegExp(/^\d{11}$/);

    telValid = telCheck.test(telephone);
    mailValid = emailCheck.test(mail);

    if (telValid) {
        if (mailValid) {
            console.log("both is right.");
        } else {
...
function testRedundancy(str) {
    ...
    while ((part = pattern.exec(str)) !== null) {
        result.add(part[0]);
        pattern.lastIndex -= (part[0].length - 1) / 2;
    }
...
function testKeyBoard(wantInput, actualInput) {
    var want = new Set(wantInput.toUpperCase()),
        actual = new Set(actualInput.toUpperCase());
